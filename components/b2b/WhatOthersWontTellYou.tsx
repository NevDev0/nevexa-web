...
import { whatOthersWontTellYou as whatOthersWontTellYouEn } from "@/content/b2b.en";
import { whatOthersWontTellYou as whatOthersWontTellYouFr } from "@/content/b2b.fr";

export default function WhatOthersWontTellYou() {
  const { language } = useLanguage();
  const whatOthersWontTellYou = language === "fr" ? whatOthersWontTellYouFr : whatOthersWontTellYouEn;

  const industryNormLabel = language === "fr" ? "La norme du marché" : "The Industry Norm";
  const nevexaStandardLabel = language === "fr" ? "Le Standard Nevexa" : "Nevexa Standard";
  const caseFileLabel = language === "fr" ? "Dossier" : "Case File";

  const [visible, setVisible] = useState(false);
  const [visibleItems, setVisibleItems] = useState<boolean[]>(
    new Array(whatOthersWontTellYouEn.dossierItems.length).fill(false)
  );
